...
local function elixir_append_modules_in_folder(modules, ignored_modules, folder)
  if vim.fn.isdirectory(folder) == 1 then
    ...
    while true do
      ...
      if name:match("%.beam$") then
        ...
        if ignored_modules[module] == nil then
...
local function elixir_view_docs_with_runtime_folders(runtime_module_folders, ignored_modules, opts)
  local modules = {}
  for _, folder in ipairs(runtime_module_folders) do
    elixir_append_modules_in_folder(modules, ignored_modules, folder)
  end
...
-- i have tried to find out which modules should be ignored by asking elixir to read the info from
-- the beam files, see the apidocs_filter_hidden branch. However the beam files are not that simple
-- to parse apparently and so that was hopelessly slow. Using ripgrep to parse the source is much faster,
-- even though it's not really correct... We depend on the formatting in the code, and for absinthe
-- at least we miss some autogenerated modules (I'm not sure whether moduledoc false applies to submodules or not)
local function get_hidden_module_list(module_paths, opts)
  local ignored_modules = {}
  local res = vim.fn.jobstart({"sh", "-c", 'rg -I "moduledoc false" -B2 ./deps | grep -h "^defmodule\\|^defprotocol"'}, {
    cwd = '.',
    stdout_buffered = true,
    on_stdout = vim.schedule_wrap(function(j, output)
      for _, line in ipairs(output) do
        ignored_modules[string.gsub(string.gsub(string.gsub(line, "defmodule ", ""), " do", ""), "defprotocol ", "")] = true
      end
    end),
    on_exit = vim.schedule_wrap(function(j, output)
      elixir_view_docs_with_runtime_folders(module_paths, ignored_modules, opts)
    end),
  })
end
...
local function elixir_view_docs(opts)
  ...
  vim.fn.jobstart({
    ...
    on_exit = vim.schedule_wrap(function(j, output)
      ...
      get_hidden_module_list(module_paths, opts)
    end),
  })
end
